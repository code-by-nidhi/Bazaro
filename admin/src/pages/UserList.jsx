      ...
      <div className="space-y-6">
        {loading ? (
          ...
          <div className="md:hidden space-y-3">
            {users.map((usr) => (
              <div key={usr._id} className="bg-white border border-slate-200 rounded-2xl p-4 shadow-xs space-y-3 text-xs">
                <div className="flex items-center gap-3">
                  <img src={usr.avatar?.url} alt={usr.name} className="w-10 h-10 rounded-full object-cover border border-slate-200 shrink-0" />
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-[11px] text-slate-400">{usr.phone || 'No phone'}</p>
                  </div>
                </div>
